...
let myNum: any = 5; // TODO: togli questo any ASAP
myNum = "5";
...
let newVar: string; // in questo caso typescript non poteva sapere che tipo di dato sarebbe stato assegnato in futuro, perché non aveva un valore iniziale.
...
const addition2 = (n1: number, n2: number): string => {
  const sum = n1 + n2;
  return sum.toString();
};
...
if (maybe === true) {
  const myBoolean: boolean = maybe; // maybe in qunto true può essere assegnato ad una variabile che accetti valori booleani
  // const anotherVar: number = maybe; // errore perché se siamo qui dentro maybe è un valore di tipo boolean
}

if (maybe === "una stringa") {
  const myString: string = maybe; // il tipo di maybe diventa il valore letterale "una stringa" che è accettato da un contenitore che si aspetta stringhe
}
...
const myArrInferred2: (string | number)[] = [];
...
const myArrInferred3: StringOrNumber[] = [];
myArrInferred3.push(2);
myArrInferred3.push("uno");

const myArrInferred4: (string | undefined)[] = [];
...
const myTupleOfThree: [number, string, boolean] = [0, "1", true];
const myTupleOfFour: [number, string, boolean, string] = [0, "1", true, "2"];
...
// potrebbe nascere l'esigenza di mantenere una certa coerenza tra un oggetto e l'altro, per vari motivi.
...
// in questo type custom yearsOfExp è una proprietà facoltativa, ci è anche permesso ometterla nell'oggetto che creeremo a partire da questo type
type RemoteTeacher = {
  role: string;
  yearsOfExp?: number;
  active: boolean;
...
const teacher1: Person & RemoteTeacher = {
  name: "Stefano",
  surname: "Miceli",
  age: 34,
...
const teacher2: Person & RemoteTeacher = {
  name: "Stefano",
  surname: "Casasola",
  age: 35,
  role: "frontend teacher",
...
interface EpicodeStudent extends HumanBeing {
  hasWebcam: boolean;
  batchName: string;
  preferredTopic?: "HTML" | "CSS" | "JS" | "SASS" | "React" | "Redux" | "TypeScript";
}
...
// ma mi permette di ammettere un qualsiasi ti po per una o più proprietà del mio oggetto
// il tipo verrà specificato al momento dell'assegnazione dell'interfaccia (vedi creazione degli oggetti qui di seguito...)

// interfaccia con due parametri di tipo
interface EpicodeUnit<N, T> {
  name: string;
  //   assignedTeacher: string | string[];
  assignedTeacher: N; // valore di tipo che arriva da fuori, nell'utilizzare l'interfaccia, tramite parentesi angolari
  //   topic: string | string[] | Topic[];
  topic: T; // valore di tipo che arriva da fuori, nell'utilizzare l'interfaccia, tramite parentesi angolari
}
...
const U3: EpicodeUnit<string, Topic[]> = {
  name: "Unit3",
  assignedTeacher: "Stefano Miceli",
  topic: [
    {
